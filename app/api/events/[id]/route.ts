...
import { deleteEvent, getEventById, updateEvent } from "@/lib/data";
...
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    const action =
      body.status !== undefined && Object.keys(body).length === 1
    ...
    await writeAuditLog({
      ...
      summary:
        action === "event.publish"
          ? `Published event “${updated.title}”`
          : action === "event.unpublish"
            ? `Unpublished event “${updated.title}”`
            : `Updated event “${updated.title}”`,
      metadata: { status: updated.status, changes: Object.keys(body) },
    });
